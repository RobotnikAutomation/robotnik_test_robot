...
int main(int argc, char** argv){
  ...
  float side_size, max_vel_x, max_vel_y, max_vel_theta, acc_lim_x, acc_lim_y, acc_lim_theta;
  bool omni_movement;
  ros::NodeHandle nh("~");
  ...
  nh.param<bool>("omni_movement", omni_movement, false);
  nh.param<float>("max_vel_x", max_vel_x, 2.0);
  nh.param<float>("max_vel_y", max_vel_y, 2.0);
  nh.param<float>("max_vel_theta", max_vel_theta, 1.0);
  nh.param<float>("acc_lim_x", acc_lim_x, 1.0);
  nh.param<float>("acc_lim_y", acc_lim_y, 1.0);
  ...
  if(omni_movement){
	  ...
	  ss << max_vel_y;
	  aux = ss.str();
	  system(("rosrun dynamic_reconfigure dynparam set /move_base/TebLocalPlannerROS max_vel_y "+aux).c_str());
	  ss.str("");
	  ss.clear();
	  ss << acc_lim_y;
	  aux = ss.str();
